...
> ⚠️ **対象の `check_async.py` は本リポジトリに存在しない**（2026-09-24 に git 履歴全体を確認。一度も追跡されていない）。本書は他プロジェクトの学習用スクリプトの記述が紛れ込んだもので、現行の `chunking/` の仕様ではない。現行の非同期処理は [`async_api_client.md`](async_api_client.md) を参照。
...
| 1.1 | 対象の `check_async.py` が本リポジトリに存在しない旨を概要に注記（2026-09-24） |
