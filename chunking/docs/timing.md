...
62.7 秒/ブロックは**正しく動いたときの速度**であって、速くはない。

| 対象 | ブロック数 | Step1 の目安 |
|---|---|---|
| 20 行 | 55 | 約 1 時間 |
| 500 行（`cc_news_2per.csv` 全件） | 約 1,375 | **約 24 時間** |

Step2 / Step3 が加わるのでさらに増える。全件を現実的な時間で回すには:

- **軽いモデルにする** — `llama3.2:latest`（2.0 GB）は `gemma4:12b-mlx`
  （7.7 GB）よりかなり速い。品質とのトレードオフは実際に出力を見て決める
- **`OLLAMA_NUM_PARALLEL` を上げる** — メモリに余裕があるとき
- **`--max-rows` で刻む** — `--resume` と併用して分割実行
