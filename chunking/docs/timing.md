...
2. [なぜ測るのか](#1-なぜ測るのか)
3. [20 行で測る](#2-20-行で測る)
4. [2.5 基準値（実測・2026-09-11）](#25-基準値実測2026-09-11)
5. [2.6 3 段階すべての実測（2026-09-11・初の完走）](#26-3-段階すべての実測2026-09-11初の完走)
6. [数字の読み方](#3-数字の読み方)
7. [workers を変えて比べる](#4-workers-を変えて比べる)
8. [まず診断スクリプトを回す](#5-まず診断スクリプトを回す)
9. [5.4 そもそも Ollama が動いていないとき](#54-そもそも-ollama-が動いていないとき)
10. [5.5 スキーマをオウム返しされたとき](#55-スキーマをオウム返しされたとき)
11. [失敗しているときに見る順番](#6-失敗しているときに見る順番)
12. [関連する環境変数](#7-関連する環境変数)
13. [変更履歴](#8-変更履歴)
...
> **種別 B**（`a_cross_doc_md_format.md` §6。手順書）。**状態: 現行（計測・切り分けの手順と実測値）**

### 結論

- チャンク化が「遅い」のか「壊れている」のかは、1 ブロックあたりの秒数で切り分ける
- `gemma4:12b-mlx` / `--block-size 1000` の基準値は**約 62.7 秒**（60 秒台なら健全、2026-09-11 実測）
- 180 の倍数（180 / 360 / 543 秒）はタイムアウトであり、1 件も成功していない（§3）
- 失敗しているときは、まず `chunking.diagnose_ollama` を回す（§5）

### 対象モジュール

| # | モジュール | 関係 |
|---|---|---|
| 1 | `chunking/csv_text_to_chunks_text_csv.py` | 計測対象のチャンク化 CLI |
| 2 | `chunking/async_api_client.py` | タイムアウト・リトライ・中断 |
| 3 | `chunking/diagnose_ollama.py` | Ollama の疎通・速度診断（§5） |
