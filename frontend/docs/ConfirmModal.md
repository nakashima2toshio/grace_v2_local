...
**なし。** 送信中フラグすら親が持つ（`submitting` prop）。

> 📌 2026-09-21 にフォーカストラップを入れたため、`useRef`（`modalRef`）と
> `useEffect`（初期フォーカス）は持つようになった。**表示される値を持つ state は
> 引き続きゼロ**で、参照はモーダル要素の DOM ノードだけである。
...
| # | 依存配列 | 内容 |
|---|---|---|
| 1 | `[]`（マウント時 1 回） | モーダル内の最初のフォーカス可能要素（承認ボタン）へ `focus()` する |

**スクロールロックと `Escape` キー購読は引き続き実装していない**
（`Escape` は §8 のとおり意図的）。

Tab の閉じ込めは `useEffect` ではなく、モーダル要素の `onKeyDown` で行う。
移動先の計算だけを `state/focusTrap.ts` の純関数（`isTabKey` / `nextFocusIndex`）へ出し、
`focus()` の呼び出しと `document.activeElement` の参照はコンポーネント側に残してある
（CLAUDE.md §6 — node 環境の vitest では DOM を触れないため）。
...
| モーダルにフォーカストラップがあるか | ✅ **実装済み**（2026-09-21）。Tab / Shift+Tab は端で巻き戻り、背面へ抜けない。移動先の計算は `state/focusTrap.ts::nextFocusIndex` |
| 開いたときにモーダル内へフォーカスが移るか | ✅ **実装済み**（2026-09-21）。マウント時に最初の要素（承認ボタン）へ `focus()` |
| 閉じたときに元の要素へフォーカスが戻るか | ❌ 未実装。**モーダルを閉じるのは承認／拒否のときだけ**で、直後に画面の内容自体が変わる（実行結果・エスカレーション）ため、戻し先が自明でない |
| `Escape` で閉じられるか | ❌ **意図的に閉じない**（§6.1）。承認／拒否のどちらかを明示的に選ぶ関門で、「閉じる」に相当する既定の選択肢が無い（拒否へ倒すと誤操作で実行機会を失い、承認へ倒すのは論外）。タイムアウト時はバックエンドが安全側へ倒す |
| キーボードのみで承認・拒否できるか | ✅ ネイティブ `<button>` で Tab + Enter/Space。到達性はフォーカストラップで担保される |
...
> 📌 残る ❌ 2 件は**いずれも判断のうえで未対応**（`Escape` は意図的、フォーカス復帰は
> 戻し先が自明でない）であり、単なる実装漏れではない。

> ⚠️ **`disabled` なボタンはトラップ対象から外れる。** 送信中（`submitting=true`）は
> 両ボタンとも `disabled` になるため対象が 0 件になり、`nextFocusIndex` は -1 を返して
> **Tab の既定動作に任せる**（押さえ込むべき焦点がそもそも無いため）。
