...
ollama serve
ollama pull gemma4:e4b               # 派生元
printf 'FROM gemma4:e4b\nPARAMETER num_ctx 8192\n' > /tmp/Modelfile
ollama create gemma4-e4b-ctx8k -f /tmp/Modelfile   # 既定モデル（config.py::get_default_ollama_model() 参照）
...
| **それ以外の全 LLM 用途**（Q&A生成・Plan/Execute/Reasoning/Confidence/Replan/ReAct 等） | **ローカル LLM（Ollama）** | `gemma4-e4b-ctx8k`（軽量も同一） | **不要** |
...
# OLLAMA_DEFAULT_MODEL=gemma4:26b-a4b-it-qat  # 既定 gemma4-e4b-ctx8k を変えるときだけ
...
| デフォルトモデル | `gemma4-e4b-ctx8k`（`config.py::get_default_ollama_model()` 参照） | `claude-sonnet-4-6`, `gpt-4o-mini`, `gemini-2.5-flash` |
