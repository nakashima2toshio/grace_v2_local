...
class ReasoningTool(BaseTool):
    ...
    def _build_prompt(
        ...
        # 回答のルール
        #
        # ⚠️ 規則 3〜5 は実測の誤りをそれぞれ 1 つずつ潰すためにある。
        #    「丁寧に書け」の類ではないので、短くしようとして削らないこと。
        #    根拠は下の各コメントを参照。
        prompt_parts.append(
            "\n### 【回答の構成ルール（最重要）】\n"
            ...
            # 種別（社内/Web）の偽装対策。実測: Yahoo!天気の内容に
            # 「社内ナレッジ（web_search）によると」と書いた。
            ...
            # Web 情報源どうしの取り違え対策。実測 2 件:
            #   - 情報源 7（Yahoo）の文章に情報源 8（tenki.jp）の URL を付けた
            #   - 「webath.co.jp」という実在しないドメインを書いた
            #     （正しくは weathernews.jp）
            # どちらも「サイト名」を許したことで記憶から補う余地が生まれている。
            # URL の丸写しを求めることで、対応付けを記憶ではなく転記の作業にする。
            "4. **出典は引用元から書き写す（記憶で書かない）**: 1 つの記述には、"
            "その内容が実際に載っている情報源を 1 つだけ対応させ、"
            "その情報源の「出典:」行を**そのまま省略せずに**書き写してください。\n"
            "   ⚠️ 複数の情報源の内容を 1 つの箇条書きに混ぜないでください。\n"
            "   ⚠️ サイト名やドメインを記憶から補わないでください。"
            "「出典:」行に無い URL・ドメイン名を書くことは捏造にあたります。\n"
            # 内部番号の露出対策。実測:
            #   「明日の具体的な予報については、別の情報源（情報源7）で…」
            # 「情報源7」はこのプロンプト内部の通し番号で、回答を読む人には
            # 何のことか分からない。
            "5. **情報源番号を書かない**: 「情報源 1」「情報源 7」のような番号は"
            "こちらの内部の通し番号です。回答を読む人には見えないので、"
            "本文で参照しないでください（代わりに出典の URL やファイル名を書きます）。\n"
