...
class ExecutorConfig(BaseModel):
    ...
    #
    # ⚠️ **この値は RAG 検索側の「採用の下限」も兼ねる**
    #    （`grace/tools.py::RAGSearchTool.execute` の緩和結果の採用判定）。
    #
    #    不変条件: **推論に使えない文書は、出典としても採用しない。**
    #
    #    以前は 2 箇所が食い違っていた。緩和閾値（0.5）でしか拾えなかった
    #    0.53 の無関係文書がスコア無条件で採用され、reasoning からはこの値で
    #    弾かれる——つまり**回答に 1 文字も寄与しないのに出典一覧にだけ載る**
    #    状態だった（実測「明日の東京の天気は？」で
