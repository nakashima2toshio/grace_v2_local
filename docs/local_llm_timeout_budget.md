...
- [概要](#概要)
- [1. `openai._base_client` は不具合ではない](#1-openai_base_client-は不具合ではない)
- [2. 予算はリトライ込みで数える](#2-予算はリトライ込みで数える)
- [3. 補助 LLM 判定は既定で切る](#3-補助-llm-判定は既定で切る)
- [3.5 本文が空になる本当の理由 — 思考しか返っていない](#35-本文が空になる本当の理由--思考しか返っていない)
- [3.6 失敗の増幅を止める](#36-失敗の増幅を止める)
- [4. 検証器の失敗で回答を捨てない](#4-検証器の失敗で回答を捨てない)
- [5. fastembed の警告が大量に出る件](#5-fastembed-の警告が大量に出る件)
- [6. 関連ファイル](#6-関連ファイル)
- [7. 旧既定モデル `gemma4-e4b-ctx8k` について](#7-旧既定モデル-gemma4-e4b-ctx8k-について)
- [8. 社内ナレッジに無い話題で無関係文書が出典に載る件](#8-社内ナレッジに無い話題で無関係文書が出典に載る件)
- [9. 「明日」が解決できない件 — reasoning プロンプトに現在日付が無い](#9-明日が解決できない件--reasoning-プロンプトに現在日付が無い)
- [10. 採用閾値 0.55 の妥当性 — 測り方](#10-採用閾値-055-の妥当性--測り方)
- [11. 出典の帰属を偽らない（P0-1）](#11-出典の帰属を偽らないp0-1)
- [12. Web ステップの信頼度が静かに壊れていた（P1-2）](#12-web-ステップの信頼度が静かに壊れていたp1-2)
- [13. 同じ検証・同じ埋め込みを繰り返さない（P1-1 / P1-3）](#13-同じ検証同じ埋め込みを繰り返さないp1-1--p1-3)
- [14. コンソール出力を二重にしない（P2-1）](#14-コンソール出力を二重にしないp2-1)
- [15. 閾値の実測結果 — 問題は閾値ではなくスコープだった](#15-閾値の実測結果--問題は閾値ではなくスコープだった)
- [16. 残る課題](#16-残る課題)
- [17. 出典の対応付け — 種別の次に残った 2 つの誤り](#17-出典の対応付け--種別の次に残った-2-つの誤り)
- [変更履歴](#変更履歴)
...
- 同じ質問の所要時間は **1:17:46 → 1:58** になった。決め手は思考抑止（`reasoning_effort=none`・§3.5）で、思考だけ返って本文が空になる状態を止めた
- LLM の呼び出し回数は変わっていない。効いたのは **1 呼び出しの所要時間**と、それを前提にした**予算設計**（リトライ込みで数える・§2）
- ローカル LLM では補助 LLM 判定を既定で切り（§3）、検証器が失敗しても回答を捨てない（§4）
- RAG の採用閾値は 0.64（暫定）。誤った出典が載る問題は**閾値ではなく検索スコープ**が原因だった（§15）
- 残る課題は §16 にまとめてある
...
| # | モジュール | 関係 |
|---|---|---|
| 1 | `helper/helper_llm.py` | Ollama クライアント（期限・リトライ・思考抑止・空応答診断） |
| 2 | `grace/config.py` / `config/grace_config.yml` | 予算の既定値と実効値（`LLMConfig` / `PlannerConfig` / `JudgeConfig`） |
| 3 | `grace/confidence.py` / `backend/app/core/gates.py` | 根拠検証の失敗の扱いと、回答ゲート・救済 |
| 4 | `grace/planner.py` / `grace/replan.py` | LLM 計画生成の無効化・冗長な検索ステップの除去 |
| 5 | `grace/tools.py` | reasoning プロンプト（現在日時・出典の種別）と検索スコープ（`_apply_excluded_collections`） |
| 6 | `scripts/measure_rag_threshold.py` | 採用閾値の実測 |

全体の対応表は §6（関連ファイル・環境変数・テスト）にある。
...
| — | Web 情報源どうしの誤帰属 | 規則を追加（§17）。**効果は未検証** |
| — | 回答に内部番号が露出（「情報源7」） | 規則を追加（§17）。**効果は未検証** |
...
## 17. 出典の対応付け — 種別の次に残った 2 つの誤り

§11 で種別（社内 / Web）の偽装は直った。その**後に**残ったのがこの 2 つ。
実測（除外・閾値を入れた後の「明日の東京の天気は？」）で観測した。
...
この文言は情報源 8（tenki.jp）ではなく **情報源 7（Yahoo!天気）** のもの。
種別は合っているが、**どの Web 情報源かが違う**。

同じ根の誤りとして、除外前の実行では `webath.co.jp` という**実在しない
ドメイン**を出典として書いていた（正しくは `weathernews.jp`）。

原因はどちらも規則が「サイト名**または** URL」と書いていたこと。
サイト名を許すと、モデルが記憶から補う余地が残る。

**対処:** 「その情報源の『出典:』行を**そのまま省略せずに**書き写す」に変えた。
対応付けを記憶ではなく**転記の作業**にする。あわせて
「1 つの記述に情報源を 1 つだけ対応させる」「複数の情報源を 1 つの箇条書きに
混ぜない」を入れた。

### 誤り② 内部の通し番号が回答に出る

> 明日の具体的な予報については、別の情報源（**情報源7**）で
> 「今日・明日の天気」として確認できる旨が示されています。

「情報源 7」はこのプロンプト内部の通し番号で、回答を読む人には何のことか
分からない。

**対処:** 番号自体はモデルが情報源を区別するのに有用なので**残し**、
「本文で参照しない（代わりに出典の URL やファイル名を書く）」と明示した。

### ⚠️ これはプロンプト規則の追加であって、保証ではない

規則を足したからといってモデルが従うとは限らない。テストが固定しているのは
**規則がプロンプトに載っていること**だけで、出力が正しくなることではない。
効果は次の実行ログで確認する。

⚠️ 規則 3〜5 はいずれも「実測の誤りを 1 つずつ潰すため」に存在する。
