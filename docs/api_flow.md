...
| サブシステム | ファイル | 概要 |
|---|---|---|
| **GRACE-Review**（文書 → 指摘） | `api/review.py`, `core/review_agent.py`, `core/review_gates.py`, `core/rulesets.py` | 文書レビュー。判定の骨格は Support と同型に作られているが、**コードは共有していない別コア**（`run_review_agent_core`）。設計は `backend/docs/review_flow.md` |
