...
**モードごとの違いと、どのガードレールが効くかの一覧は `docs/pipelines.md` を先に読むこと。**
本書はそこで挙がった機構 GA〜G9 の中身を説明する。回答生成そのものは `docs/reasoning_flow.md`。
...
`grace/intervention.py` / `support_actions.py` /
`backend/app/core/review_gates.py` / `backend/app/core/review_agent.py`
...
| ID の系統 | 基本版 | GRACE-Support | GRACE-Review |
|---|:--:|:--:|:--:|
| GA / GA'（入力の分析） | GA のみ | GA・GA' | — |
| G0〜G6（取得・検証・ゲート） | ✅ | ✅ | 同型の別実装（§3.2） |
| G7〜G9（アクション） | G7・G9 | ✅ | G7・G9 |

**基本版は `vertical=None` で同じコアを通る。** 業界プロファイル由来のガードレール
（GA'・G3 のキーワード・G8）はすべて無効になる（`docs/pipelines.md` §3）。
...
flowchart TB
    ...
    subgraph SA["0-(A) 入力・質問分析"]
        GA["GA 複数質問の二段判定 looks_like_multi_question → create_question_analyzer"]
        GA1{"主質問が複数か"}
...
| G0 | RAG 採用下限 | `config/grace_config.yml` の `executor.reasoning_min_rag_score` | コサイン類似度 0.64 未満の RAG 結果は reasoning にも出典にも使わない。全件除外になる場合はフィルタ不適用 | フィルタ無効（0 件化を避ける） |
| G1 | 根拠検証 | `grace/confidence.py` `GroundednessVerifier.verify` | 回答を claim へ分解し `supported`/`contradicted`/`neutral` の 3 値判定。同一入力はキャッシュ（`_CACHE_SIZE=4`）で再検証しない | `verification_failed=True` を立てて後段の救済判断へ回す |
...
| G1C | 矛盾による cap | `grace/executor.py` `_blend_groundedness_confidence` | 矛盾が 1 件でもあれば `answer_conf` を 0.30 で頭打ち | — |
| G1D | 検証器障害の切り分け | `grace/confidence.py` `GroundednessResult` | 「肯定できなかった」と「検証器が落ちた」を `verification_failed` で区別 | — |
| G2 | 回答ゲート | `backend/app/core/gates.py` `_answer_gate` | `verified` かつ 出典 1 件以上 かつ 支持率が閾値以上で `answer`。`notify` 以上＝注記なし、`confirm` 以上＝未確認注記つき | 未検証・出典 0 → `escalate` |
| G3 | 強制エスカレ（二段判定） | `gates.py` `_should_force_escalate` | 第 1 段＝`escalate_keywords` 部分一致、第 2 段＝意図分類。`question`（FAQ 質問）なら誤検知として抑止 | 分類失敗 → 安全側で強制エスカレ |
| G4 | 未肯定回答の救済 | `gates.py` `_should_rescue_unaffirmed` | 矛盾なし・出典あり・実質回答なら、支持率が弱いだけでは捨てず未確認注記つきで維持 | 強制エスカレ時は救済しない |
...
| モジュール | 役割 | 主要シンボル |
|---|---|---|
| `backend/app/core/gates.py` | **回答ゲート判定の純ロジック群**。CLI 版と判定を一致させるため副作用を持たない | `_answer_gate` `_should_force_escalate` `_detect_no_info_answer` `_should_rescue_unaffirmed` `_should_rescue_unverified` `_decide_action` `_match_keyword` `judge_model` `judges_enabled` |
| `backend/app/core/support_agent.py` | パイプライン中核。①〜⑥＋④'＋救済を配線し `SupportEvent` を emit | `run_support_agent_core` `_perform_action` `SupportResult` `STEP_IDS` |
| `backend/app/core/verticals.py` | 業界プロファイル。閾値・エスカレ語・アクション対応・検索スコープ・スコープ方針 | `VerticalProfile` `PROFILES` `SCOPE_POLICY` `INTENT_MODEL` `JUDGE_MAX_OUTPUT_TOKENS` |
| `grace/confidence.py` | **評価器の本体**。根拠検証・自己評価・網羅度・一致度・信頼度合成 | `GroundednessVerifier` `GroundednessResult` `ConfidenceCalculator` `LLMSelfEvaluator` `SourceAgreementCalculator` `QueryCoverageCalculator` `ConfidenceAggregator` `InterventionLevel` |
| `grace/executor.py` | 実行と信頼度合成。`_blend_groundedness_confidence` と M-6 減衰（`_damp_support_rate`） | `_blend_groundedness_confidence` `_damp_support_rate` |
| `grace/intervention.py` | HITL。CONFIRM / ESCALATE / NOTIFY / SILENT の 4 レベルと閾値の動的調整 | `InterventionHandler` `DynamicThresholdAdjuster` `ConfirmationFlow` |
| `backend/app/core/intervention_bridge.py` | Web の承認待ちを解決（CLI の自動承認を Web に持ち込まないための境界） | `InterventionBridge` `PendingIntervention` |
| `support_actions.py` | 本人確認とアクション実行バックエンド | `IdentityVerifier` `CsvIdentityChecker` `DryRunActionBackend` `WebhookActionBackend` |
| `grace/calibration.py` | 信頼度の温度スケーリング較正（ECE 計測付き） | `Calibrator` `fit_temperature` `expected_calibration_error` |
| `grace/replan.py` | 失敗・低信頼時のリプラン戦略決定 | `ReplanManager` `ReplanOrchestrator` `ReplanTrigger` |

### 3.2 GRACE-Review（文書 → 指摘）の同型ガードレール

⚠️ **「副系統」ではない。** Review はタブとして独立した 1 モードであり（`docs/pipelines.md` §1）、コアも `run_review_agent_core` で別である。ただし**判定の骨格は Support と同型**に作ってある — `review_gates.py` の docstring に「`gates.py` を『回答 → 指摘』へ読み替えた版」と明記されている。

| Support（`gates.py`） | Review（`review_gates.py`） | 概要 |
|---|---|---|
| `_match_keyword` | `select_candidate_rules`（そのまま再利用） | 第 1 段のキーワード候補検出 |
| `_answer_gate` | `decide_finding_status` | 指摘の採否判定 |
| `_should_force_escalate` | `should_force_high` | 重大リスク語による強制 high |
| `_detect_no_info_answer` | `detect_vacuous_finding` | 実質性のない指摘の検知 |
| `_should_rescue_unaffirmed` | `should_rescue_finding` | 支持が弱い指摘を消さず `review_required` へ |
| `create_intent_classifier` | `create_mention_classifier` | 第 2 段の LLM 分類 |
| `create_no_info_judge` | `create_vacuous_judge` | 第 2 段の YES/NO 判定 |

関連: `backend/app/core/rulesets.py`（`RuleItem` / `RuleSet`）、`backend/app/core/review_agent.py`。

Review における「安全側」は **指摘を消さない（`review_required` にする）** であり、
Support の「回答せず escalate」と同じ考え方（誤って人に届けないより、人に確認してもらう方が損失が小さい）。

#### Review 固有の落とし穴（実測 2026-08-31）
...
（`gates.py` の `create_intent_classifier` が `lambda _query: None` を返す）。
