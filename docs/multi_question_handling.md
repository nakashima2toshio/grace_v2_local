...
そこで `VerticalProfile.build_prompt_addendum(out_of_scope_questions)` が、
範囲外の**質問文だけ**を reasoning の業務方針へ足す:

```
【この問い合わせに含まれる担当範囲外の質問】
- 明日の東京の天気は？
これらには内容を回答せず、**同じ回答の中で**担当範囲外である旨を明示したうえで、
次の案内を添えること: 天気・ニュース・一般常識や他機関の手続きは…
担当範囲内の質問には通常どおり回答し、範囲外の分もまとめて 1 回の回答で扱うこと
（別の問い合わせとして先送りしない）。
```
...
ローカル LLM が落としたのは、回答生成プロンプトの【回答の構成ルール】
1（参照情報にある事実のみ）・7（捏造禁止）と衝突して見えるためと考えられる。
注入文でも「担当範囲の案内はその例外」と明示したが、それでも従う保証はない。
...
姉妹リポジトリ（grace_v2・クラウド LLM）では**モデル自身が断りを書くのが通常の
経路**になった（実測 2026-08-31）。本リポジトリのローカル LLM は同じ実行で
断りを書かず定型文の追記で担保されたが、モデルを変えれば主経路も変わる。
「マーカーがあれば何もしない」の分岐でモデルが URL を落とすと案内先が丸ごと
消えるので、足りない **URL だけ**を足す（定型の断り文を重ねない）。
...
v3.0 で実装した**絞り込み方式**（§0）の実体。§1 以降の 3 案は採用していない。

| # | モジュール | 関係 |
|---|---|---|
| 1 | `backend/app/core/gates.py` | `looks_like_multi_question`（第 1 段・LLM 呼び出しゼロ）→ `create_question_analyzer`（分解と担当範囲を 1 回の LLM で）→ `split_by_scope` → `reconstruct_query` |
| 2 | `backend/app/core/support_agent.py` | `STEP_IDS` の `analyze` ステップ（0-(A)）として上記を配線し、主質問の選択を HITL で待つ |
| 3 | `backend/app/core/intervention_bridge.py` | Web の承認待ち（主質問の選択）を解決する |
| 4 | `frontend/src/components/QuestionSelectModal.tsx` / `frontend/src/state/interventionKind.ts` | 主質問を選ばせるモーダルと、承認待ちが action か question かの判定 |
