...
2. [現状の使用状況](#1-現状の使用状況)
3. [技術的比較](#2-技術的比較)
4. [長所・短所の比較](#3-長所短所の比較)
5. [使用場面による選択](#4-使用場面による選択)
6. [パフォーマンス比較](#5-パフォーマンス比較)
7. [統一すべきか？](#6-統一すべきか)
8. [推奨アーキテクチャ](#7-推奨アーキテクチャ)
9. [移行シナリオ（参考）](#8-移行シナリオ参考)
10. [最終推奨事項](#9-最終推奨事項)
11. [まとめ表](#10-まとめ表)
12. [変更履歴](#11-変更履歴)
...
> **種別 B**（`a_cross_doc_md_format.md` §6）。**状態: 判断済み（チャンク分割は asyncio、Q/A 生成は Celery の併用を維持）**

### 結論

- チャンク分割（`chunking/`）は asyncio＋セマフォ、Q/A 生成は Celery ワーカーと、**用途で並列方式を使い分けている**
- I/O 待ちが主で単一プロセスで足りるチャンク分割は asyncio、長時間・大量で再試行やワーカー分散が要る Q/A 生成は Celery が向く
- **統一せずハイブリッド構成を維持する**のが推奨（§6「統一すべきか？」・§9「最終推奨事項」）
- 本文中の `csv_to_chunks_text_para.py` は現在 `chunking/csv_text_to_chunks_text_csv.py`（＋`chunking/async_api_client.py`）である

### 対象モジュール

| # | モジュール | 関係 |
|---|---|---|
| 1 | `chunking/csv_text_to_chunks_text_csv.py` / `chunking/async_api_client.py` | asyncio による並列 API 呼び出し |
| 2 | `qa_generation/pipeline.py`（`_generate_with_celery()`） | Celery へのタスク投入 |
| 3 | `celery_tasks.py` / `celery_config.py` | Celery タスク・設定 |
