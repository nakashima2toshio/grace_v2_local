...
|---|---|---:|---:|---|:--:|
| [`pipeline.md`](pipeline.md) | `pipeline.py` — `QAPipeline`（Web / CLI 共通の実体） | 549 | 725 | — | ★★★ |
| [`smart_qa_generator.md`](smart_qa_generator.md) | `smart_qa_generator.py` — `SmartQAGenerator`（構造化出力 1 回） | 301 | 497 | — | ★★★ |
| [`semantic.md`](semantic.md) | `semantic.py` — `SemanticCoverage`（Embedding によるカバレージ） | 542 | 710 | — | ★★☆ |
...
| 2 | 3 文書に Anthropic 前提の記述が残る（§4）。実装は Ollama 済み | 中 |
| 3 | `pipeline.py:341` の `provider="anthropic"`（死んだ引数・§4.1）。受け側ごと消せるか要確認 | 低 |
| 4 | 4 文書とも `**Version X.X**` ヘッダーが無い | 低 |
