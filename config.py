...
def get_default_chunking_workers() -> int:
    ...
    ## ⚠️ ローカル LLM で並列数を上げても速くならない（実測で確定）

    既定は長らく 8 だった。クラウド API（Gemini / Anthropic）時代の値で、
    そちらは同時リクエストを本当に並列処理するため 8 は妥当だった。
    **ローカルの Ollama は既定で 1 本ずつしか処理しない。**
    ...
    ＝ 約 73 秒/ブロックで、単発の実測 62.7 秒とほぼ同じ。つまり並列度を
    上げて得られた速度は **ゼロ**。

    それどころか害がある。残り 7 本はキューで待つだけなのに、その待ち時間が
    **各自のタイムアウトを食いつぶす**。順番が来ないまま期限切れになり、
    機械的分割のフォールバックへ落ちる。`workers=8` が生んだものは
    待ち行列とタイムアウトだけだった。

    ## 上げてよいのは Ollama 側も増やしたときだけ

    `OLLAMA_NUM_PARALLEL` が設定されていればその値に従う。Ollama が本当に
    N 本同時に処理するなら、こちらも N 本投げてよい（メモリは N 倍要る）。

        OLLAMA_NUM_PARALLEL=4 ollama serve

    メモリはモデルの重みが N 倍になるわけではない（重みは共有される）。
    増えるのは**スロットごとの KV キャッシュ**なので、増加量は num_ctx と
    N に比例する。とはいえ上限はあるので、上げたら `ollama ps` で実際の
    使用量を見ること。

    設定が無ければ 1。**「たぶん並列で速くなる」で上げないこと。**
    上げたら `chunking/docs/timing.md` の手順で 秒/件 を測り、実際に
    下がったことを確かめる。
    """
    raw = os.getenv("OLLAMA_NUM_PARALLEL", "").strip()
    if not raw:
        return 1
    try:
