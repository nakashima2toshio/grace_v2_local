...
> アーキテクチャ構成図〔3 層〕・変更履歴・Mermaid 黒背景）はそのまま保持し、
> IPO 詳細の部分だけを Props・状態・副作用・SSE に置き換える。対応は[付録 A](#付録-a-基本フォーマットとの対応表)。
...
3. [アーキテクチャ構成図・コンポーネントツリー図](#3-アーキテクチャ構成図コンポーネントツリー図)
...
14. [付録 A: 基本フォーマットとの対応表](#付録-a-基本フォーマットとの対応表)
15. [付録 B: 実装との整合チェックリスト](#付録-b-実装との整合チェックリスト)
16. [変更履歴（本仕様書）](#変更履歴本仕様書)
...
## 目次
## 概要                         ← メタ表・主な責務・各責務対応のモジュール・主要機能一覧
## 1. アーキテクチャ構成図
     ### 1.1 システム全体での位置づけ（3 層）
     ### 1.2 コンポーネントツリー図
## 2. Props インターフェース
## 3. 状態管理
## 4. データフロー・副作用
...
| セクション | 必須 | 省略できる条件 |
|---|:---:|---|
| 概要（主な責務・各責務対応のモジュールを含む） | ✅ | — |
| アーキテクチャ構成図 1.1 位置づけ（3 層） | ✅ | — |
| アーキテクチャ構成図 1.2 コンポーネントツリー図 | ✅ | 子を持たない葉コンポーネントは、1.1 の図で親子が読み取れれば「1.1 を参照」と 1 行書いてよい |
...
## 3. アーキテクチャ構成図・コンポーネントツリー図

`## 1. アーキテクチャ構成図` は 2 枚の図で構成する。

| 図 | 基本フォーマットでの対応 | 示すもの |
|---|---|---|
| 1.1 システム全体での位置づけ（3 層） | アーキテクチャ構成図 | 呼び出し側（親）→ 対象コンポーネント → 外部（API クライアント・バックエンド） |
| 1.2 コンポーネントツリー図 | モジュール構成図 | 親子関係と、どの階層で状態が生まれ・どの向きに流れるか |

### 3.1 システム全体での位置づけ（3 層）
...
flowchart TB
    subgraph CALLER["呼び出し側"]
        App["App.tsx"]
        Panel["SupportPanel.tsx"]
    end
    subgraph TARGET["対象コンポーネント"]
        Timeline["StepTimeline.tsx"]
        Reducer["state/jobReducer.ts<br>STEP_IDS / STEP_LABELS"]
    end
    subgraph EXTERNAL["外部（API・バックエンド）"]
        SSE["api/client.ts<br>subscribeStream()"]
        Core["backend/app/core/support_agent.py<br>STEP_IDS"]
    end
    App --> Panel
    Panel -->|"steps"| Timeline
    Timeline --> Reducer
    SSE -->|"SupportEvent"| Panel
    Core -->|"SSE step イベント"| SSE
classDef default fill:#000,stroke:#fff,color:#fff
classDef subgraphStyle fill:#1a1a1a,stroke:#fff,color:#fff
class App,Panel,Timeline,Reducer,SSE,Core default
...
1. サブグラフは「呼び出し側」「対象コンポーネント」「外部（API・バックエンド）」の 3 層を基本とする。
2. **対象が API を直接呼ばない場合も外部層は省略しない。** 表示データの出どころ
   （親経由で届く SSE・バックエンドの定数など）を外部層に置き、どこから来た値かを示す。
3. 図の直後に `### データフロー`（番号付きリスト 3〜6 行）を置いてよい。
   §6 の `useEffect` 表と重複する場合は §6 を正とし、ここは概略にとどめる。
...
| `a_class_method_md_format.md` | 本書 | 備考 |
|---|---|---|
| タイトル・Version・目次 | 同じ | 拡張子（`.tsx` / `.ts`）を付ける |
| 概要（主な責務・各責務対応のモジュール・主要機能一覧） | 同じ ＋ **メタ表** | メタ表（ファイル/種別/親/子/主な依存/対応バックエンド）は本書固有 |
| 1. アーキテクチャ構成図（3 層） | 1.1 システム全体での位置づけ | 同じ 3 層構造 |
| 2. モジュール構成図 | 1.2 コンポーネントツリー図 | 各ノードに保持 state を併記する点が本書固有 |
| 3. クラス・関数一覧表 | 概要の主要機能一覧 ＋ 6.1 イベントハンドラ一覧 | — |
| 4. クラス・関数 IPO詳細 | 2. Props ／ 3. 状態管理 ／ 4. 副作用 ／ 5. API・SSE | IPO の代わりに**契約（Props・コールバック）と状態遷移**で記述する |
| 4.1 使用例 | 2. Props のコード（親からの渡し方） | コンポーネントは JSX で使われるため、独立した使用例節は置かない |
...
- [ ] 概要に「主な責務」と「各責務対応のモジュール」があり、行数が 1:1 で一致するか
- [ ] アーキテクチャ構成図 1.1（3 層）と 1.2（ツリー）の 2 枚があるか
