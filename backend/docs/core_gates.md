...
| `ensure_out_of_scope_notice(answer, questions, guidance)` | 回答本文が担当範囲外に触れていなければ断り＋窓口案内を追記する。**モデルが指示に従わない場合の担保**（実測: Anthropic は断ったがローカル LLM は落とした）。ゲートの後で適用するので判定は動かさない |
