...
> 判定できないときは人に見せる。
>
> ⚠️ **ただし「確定（confirmed）」にはしない。** 定型文（「…に該当する可能性があります
> （自動判定に失敗したため要確認）」）はルール名を言い換えただけなので、後段の
> groundedness が条文からほぼ必ず「支持される」と判定してしまう。実測 2026-08-31 では
> 支持率 1.00 で **「自動判定に失敗したため要確認」と書かれた指摘が「確定」バッジ**で
> 表示された（適正な LP でも 9 件中 8 件）。`verdict is None` のときは status の上限を
