...
# 環境変数設定（テスト用）
#
# ⚠️ **LLM_PROVIDER をここで設定しない。** `os.environ` はプロセス全体で共有され、
# `config.py` も `helper/helper_llm.py` も **import 時**に
# `os.getenv("LLM_PROVIDER", "ollama")` を読む。conftest がこれを先に立てると、
# 同じ pytest プロセスで走る**他のテストの既定プロバイダまで**変わる。
# 移設前は `"gemini"` を立てていた（Gemini 由来コードの名残）。
# 本リポジトリの LLM は Ollama（CLAUDE.md §3）なので、既定に任せる。
#
# EMBEDDING_PROVIDER / GOOGLE_API_KEY は Embedding 用途で正しい（同 §3）。
