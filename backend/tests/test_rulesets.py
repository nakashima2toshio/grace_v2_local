...
EXPECTED_LAW_COUNTS = {
    "景品表示法": 12,
    # ⚠️ 3 → 4。yakki-04（安全性の保証表現）を追加した。実測 2026-08-20 20:18 /
    # 21:01 の NG 例で「副作用がないので誰でも安心してお使いいただけます。」が
    # 3 回とも指摘されなかった。実装の不具合ではなく、この観点を持つルールが
    # 22 件のどれにも存在しなかったための取りこぼしである。
    "医薬品医療機器等法": 4,
    "特定商取引法": 6,
    # ⚠️ 法令ルールではない整合チェック（policy-01）。grace_v2 実測 2026-08-17 20:07 で
