...
_YAKKI_RULES: List[RuleItem] = [
    ...
    # ⚠️ 実測 2026-08-20 20:18 / 21:01 の取りこぼしを埋めるために追加した。
    #    「副作用がないので誰でも安心してお使いいただけます。」が 3 回とも
    #    どのルールにも当たらなかった（22 件のどれもこの観点を持っていなかった）。
    RuleItem(
        rule_id="yakki-04",
        title="安全性の保証表現",
        category="効能効果",
        ...
        article="第66条",
        description=(
            "医薬品・医薬部外品・化粧品の広告で、安全性を保証・断定する表現は"
            "認められない。副作用や使用上の注意が個人差を伴う以上、"
            "「副作用がない」「誰にでも安全」と言い切る表示は、"
            "事実に反するか誇大な表現に当たりうる。\n"
            "指摘する: 安全性を**保証・断定**する表現。"
            "例:「副作用がない」「副作用はありません」「無添加だから安全」"
            "「誰でも安心してお使いいただけます」。\n"
            "⚠️ **指摘しない**: (a)「お肌に合わない場合はご使用をおやめください」等の"
            "**注意喚起**（むしろ適切な表示である）。"
            "(b)「安心してお買い物いただけます」のように、**商品の安全性ではなく"
            "取引・配送・サポートについて**述べているとき。"
            "(c) 承認を受けた効能効果の範囲内で客観的事実を述べているとき"
            "（例: 特定の成分を配合していない旨を、安全性の保証を伴わずに書く）。\n"
            "判定するのは『**その商品を使ううえでの安全性を、個人差を無視して"
            "言い切っているか**』だけ。「安心」「安全」という語が入っているだけで"
            "指摘してはならない。"
        ),
        # 第1段は語の有無しか見られないので広めに拾い、上の【判定基準】で
        # 注意喚起・取引上の安心と切り分ける（yakki-01 / yakki-02 と同じ建て付け）。
        keywords=[
